...
```{r}
hibs_m2 <- data.frame(
  ASGROW = c("MANTARRAYA", "KODIAK", "ALCE", "SALAMANDRA", "ANTILOPE", "BERRENDO"),
  DEKALB = c("DK2037", "DK4021", "DK4024", "DK4018", NA, NA),
  PIONEER = c("P3011W", "P32208W", NA, NA, NA, NA),
  Candidatos = c("MY8729", "VY8601", NA, NA, NA, NA)
) |> 
  pivot_longer(everything(), names_to = "brand", values_to = "commercial_name") |> 
  drop_na(commercial_name)

hibs_m2 |> count(brand, commercial_name)
hibs_m2_set1 <- hibs_m2 |> filter(!commercial_name=="VY8601")
hibs_m2_set2 <- hibs_m2 |> filter(!commercial_name=="MY8729")
```
...
traits_ME2 <- tibble(
  trait=c(
    "DINEP - PATHOGEN:Fusarium sp., TISTY:Ear",
    # "DEFG - CDEFG:Rotten",
  ...
  hib_target=c("DK2037" , "KODIAK", "DK4024", NA, "ANTILOPE"))
```
...
m2_raw <- rio::import("norla24/Norla 2 y 3.xlsx", sheet="Current_historico") |>
  janitor::clean_names()  |>  
  filter(!is.na(secondary_questions))   |> 
  filter(field_pipeline %in% "ME2 MID ALTITUDE") |> 
  unite(year_loc, c(field_year, field_name), remove = F) |> 
  unite("trait", c("obs_observation_ref_cd", "secondary_questions"), sep = " - ", remove = FALSE) |> 
  left_join(hibs_m2)

m2_raw %>% distinct(year_loc, lat=field_field_latitude, lng=field_field_longitude) -> m2_raw_geo 
m2_raw |> count(commercial_name)

m2_raw |> 
  filter(commercial_name %in% hibs_m2$commercial_name) |> 
  separate(secondary_questions, into = c("pathogen", "tisty"), 
           sep=",", remove = FALSE) |>
...
m2_full <- m2_raw |> 
  ...
  filter(!str_detect(obs_observation_ref_cd, "DINC")) |> 
  # count(pathogen, tisty)
  unite("trait", c("obs_observation_ref_cd", "secondary_questions"), sep = " - ") |> 
  filter(commercial_name %in% hibs_m2$commercial_name) |> 
  filter(trait %in% traits_ME2$trait) |> 
  left_join(traits_ME2)

m2_full |> count(brand, commercial_name)
m2_full %>% count(trait) 
...
m2_raw %>% 
  filter(commercial_name %in% hibs_m2$commercial_name) |> 
  separate(secondary_questions, 
           into = c("pathogen", "tisty"), sep=",", remove = FALSE) |>
  filter(tisty == " TISTY:Ear")  |> 
  ...
  group_by(field_year, year_loc, brand, commercial_name) |> 
  summarise(prom_obs_num_value = max(prom_obs_num_value),.groups = "drop") |> 
  mutate(prom_obs_num_value=prom_obs_num_value+1)-> rotten 
...
rotten %>% 
  group_by(year_loc) |>
  filter(c('VY8601') %in% commercial_name) %>%
  # group_by(year_loc) |>
...
rotten_mos %>% 
  ...
  labs(x="", y="", 
       ...
       title = paste("Trait:", unique(rotten$trait)[1], " ME2 - Base MD"))  
...
m_rotten <- lmer(prom_obs_num_value ~ commercial_name + (1|year_loc), data=clean_rotten)
...
```{r}
rotten_res %>% 
  # filter(commercial_name %in% hibs_m2_set1$commercial_name) |> 
  ggplot()+ 
  aes(
...
surv_data %>% 
  ...
  filter(commercial_name %in% hibs_m2_set2$commercial_name) |> 
  ggplot() + 
  geom_pointrange(aes(
...
gls %>% 
  group_by(year_loc) |> 
  filter('DK4024' %in% commercial_name) %>%
  mutate(ref = prom_obs_num_value[commercial_name=="DK4024"]) %>%
  # calcular within-field: n de hibs, FIELD_press (95°) y variabilidad
  group_by(year_loc) %>%
  reframe(ref=first(ref), 
          n_hibs=n(), 
...
spiro |> count(field_year, brand, commercial_name)
spiro %>% 
  group_by(commercial_name) %>% 
  summarise(n=n(), 
            mean_val = mean(prom_obs_num_value)) %>% 
